...
# %% Let's plot these missing values(%) vs column_names. Top 10 Features with the most missing value rate.
...
columns_to_drop = ['thumbnail_url', 'neighbourhood_group_cleansed', 'jurisdiction_names', 'xl_picture_url',
                   'medium_url', 'square_feet', 'monthly_price', 'weekly_price']
...
# Top 10 Neighbourhoods by # of Listings
...
df_popular_neighbourhoods = pd.DataFrame(list(zip(neighbourhoods, neigh_counts)),
                                         columns=['neighbourhood', 'num_listings'])

plt.figure(figsize=(16,10))
plt.bar(df_popular_neighbourhoods['neighbourhood'][:10], df_popular_neighbourhoods['num_listings'][:10], color='lightblue')
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.xlabel("Neighbourhood", fontsize=14)
plt.ylabel('Number of Listings', fontsize=14)
plt.title('Top 10 Neighbourhoods by Number of Listings', fontsize=20)
plt.show()
...
# The root mean square error calculation is: 6567.085967458968
...
# Find the prediction R2 of Linear model using 5-fold cross-validation
...
rmse = sqrt(mean_squared_error(y_test, y_test_pred))
print("The root mean square error calculation is: {}".format(rmse))
# The root mean square error calculation is: 4833.868687117079

print("RF Model Score: {}".format(rf_model.score(X, y)))
# RF Model Score: 0.8752207356935876

print("RF Model Score (Adjusted): {}".format(adjusted_r_squared(X, y, rf_model)))
# RF Model Score (Adjusted): 0.8748856157526983
...
print(rf_cross_val)
# [0.69189202 0.72718312 0.71350631 0.69606085 0.73355649]
mean_cross_val_score = rf_cross_val.mean()
print("The mean R2 score using 5-fold cross-validation is: {}".format(mean_cross_val_score))
# The mean R2 score using 5-fold cross-validation is: 0.7124397576243072
